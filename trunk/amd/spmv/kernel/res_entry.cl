                   ...
                   __global volatile int *inter,int groupnum)
{
    ...
    if(lid==0)
    {
        if(gid == 0)
#if defined AMD_GCN
            atomic_xchg(inter,column[t3][NB_LSIZE-1]);
#else
        ...
        {
#if defined AMD_GCN
            while((sum=atomic_xchg((volatile __global int *)(inter+gid - 1),INT_MAX))==INT_MAX){}
            atomic_xchg(inter+gid,column[t3][NB_LSIZE-1] + sum);
#else
