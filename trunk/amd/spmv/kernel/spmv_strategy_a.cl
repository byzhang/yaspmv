...
#if defined AMD_GCN
                         __global volatile float *inter,   //the buffer used for adjacent synchronization
#else
   ...
   unsigned int gid = get_group_id(0);
    ...
    if(lid==0){   //adjacent synchronization
#if defined BLOCK_HEIGHT_1
        if(gid ==0||t1!=0){
            atomic_xchg(inter+gid,last_partial_sums[NB_LSIZE_1+NB_LSIZE_P]);
        }
        if(gid != 0){
            while((ft1=atomic_xchg(inter+gid-1,FLT_MAX))==FLT_MAX){}
        }
        else
            ft1=0.0;
        if(gid!=0 && t1==0){
            atomic_xchg(inter+gid,ft1 + last_partial_sums[NB_LSIZE_1+NB_LSIZE_P]);
        }
...
#if defined BLOCK_HEIGHT_2
        if(gid ==0||t1!=0){
            atomic_xchg(inter+gid*2,last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s0);
            atomic_xchg(inter+gid*2+1,last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s1);
        }
        if(gid != 0){
            while((ft1.s0 = atomic_xchg(inter+(gid-1)*2,FLT_MAX))==FLT_MAX){}
            ft1.s1 = atomic_xchg(inter+(gid-1)*2+1,FLT_MAX);
        }
        else
            ft1=0.0;
        if(gid!=0 && t1==0){
            atomic_xchg(inter+gid*2,ft1.s0 + last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s0);
            atomic_xchg(inter+gid*2+1,ft1.s1 + last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s1);
        }
        previous_workgroups_last_partial_sum = ft1;   
    }
#endif
...
#if defined BLOCK_HEIGHT_3
        if(gid ==0||t1!=0){
            atomic_xchg(inter+gid*4,last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s0);
            atomic_xchg(inter+gid*4+1,last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s1);
            atomic_xchg(inter+gid*4+2,last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s2);
        }
        if(gid != 0){
            while((ft1.s0 = atomic_xchg(inter+(gid-1)*4,FLT_MAX))==FLT_MAX){}
            ft1.s1 = atomic_xchg(inter+(gid-1)*4+1,FLT_MAX);
            ft1.s2 = atomic_xchg(inter+(gid-1)*4+2,FLT_MAX);
        }
        else
            ft1=0.0;
        if(gid!=0 && t1==0){
            atomic_xchg(inter+gid*4,ft1.s0 + last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s0);
            atomic_xchg(inter+gid*4+1,ft1.s1 + last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s1);
            atomic_xchg(inter+gid*4+2,ft1.s2 + last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s2);
        }
        previous_workgroups_last_partial_sum = ft1;   
    }
#endif
        ...
        if(gid ==0||t1!=0){
            ...
            atomic_xchg(inter+gid*4+1,last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s1);
            atomic_xchg(inter+gid*4+2,last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s2);
            atomic_xchg(inter+gid*4+3,last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s3);
        }
        if(gid != 0){
            while((ft1.s0 = atomic_xchg(inter+(gid-1)*4,FLT_MAX))==FLT_MAX){}
            ft1.s1 = atomic_xchg(inter+(gid-1)*4+1,FLT_MAX);
            ft1.s2 = atomic_xchg(inter+(gid-1)*4+2,FLT_MAX);
            ft1.s3 = atomic_xchg(inter+(gid-1)*4+3,FLT_MAX);
        }
        else
            ft1=0.0;
        if(gid!=0 && t1==0){
            ...
            atomic_xchg(inter+gid*4+1,ft1.s1 + last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s1);
            atomic_xchg(inter+gid*4+2,ft1.s2 + last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s2);
            atomic_xchg(inter+gid*4+3,ft1.s3 + last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s3);
        }
        previous_workgroups_last_partial_sum = ft1;   
    }
#endif
