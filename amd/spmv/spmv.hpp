...
void generateProgramCache(clContext *clCxt)
{
    ...
    for(int dimwidth=16;dimwidth<=32;dimwidth=dimwidth*2){
        ...
        if((lt*width*bitwidth>SH_MEM_SIZE/4))  continue;
...
void transpose(clContext *clCxt,cl_mem &gdst, cl_mem &gsrc,int src_data_size,
    ...
    if(block_width<=4){ 
        while(trans_size * block_width * localthread / 1024 > 4){
            trans_size = trans_size / 2;
        }
    }
    else{
        while(block_width==32&trans_size * localthread / 1024 > 4){
            trans_size = trans_size / 2;
        }
        while(block_width==16&trans_size * localthread / 1024 > 8){
            trans_size = trans_size / 2;
        }
    }
    char build_options[200];
