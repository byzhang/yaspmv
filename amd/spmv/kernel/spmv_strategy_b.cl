                         ...
                         __global RES_TYPE *res) 
{
   ...
   unsigned int gid = get_group_id(0);
    ...
    if(lid==0){   //adjacent synchronization
        ...
        if(gid != 0){
            while((ft=atomic_xchg(inter+gid-1,FLT_MAX))==FLT_MAX){}
        }
        else
            ft=0.0;
        if(gid!=0 && t1==0){
            atomic_xchg(inter+gid,ft + last_partial_sums[NB_LSIZE_1+NB_LSIZE_P]);
        }
    }
#endif
        ...
        if(gid != 0){
            while((ft.s0 = atomic_xchg(inter+(gid-1)*2,FLT_MAX))==FLT_MAX){}
            ft.s1 = atomic_xchg(inter+(gid-1)*2+1,FLT_MAX);
        }
        else
            ft=0.0;
        if(gid!=0 && t1==0){
            atomic_xchg(inter+gid*2,ft.s0 + last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s0);
            atomic_xchg(inter+gid*2+1,ft.s1 + last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s1);
        }
    }
#endif
        ...
        if(gid != 0){
            while((ft.s0 = atomic_xchg(inter+(gid-1)*4,FLT_MAX))==FLT_MAX){}
            ft.s1 = atomic_xchg(inter+(gid-1)*4+1,FLT_MAX);
            ft.s2 = atomic_xchg(inter+(gid-1)*4+2,FLT_MAX);
        }
        else
            ft=0.0;
        if(gid!=0 && t1==0){
            atomic_xchg(inter+gid*4,ft.s0 + last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s0);
            atomic_xchg(inter+gid*4+1,ft.s1 + last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s1);
            atomic_xchg(inter+gid*4+2,ft.s2 + last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s2);
        }
    }
#endif
        ...
        if(gid != 0){
            ...
            ft.s1 = atomic_xchg(inter+(gid-1)*4+1,FLT_MAX);
            ft.s2 = atomic_xchg(inter+(gid-1)*4+2,FLT_MAX);
            ft.s3 = atomic_xchg(inter+(gid-1)*4+3,FLT_MAX);
        }
        else
            ft=0.0;
        if(gid!=0 && t1==0){
            ...
            atomic_xchg(inter+gid*4+1,ft.s1 + last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s1);
            atomic_xchg(inter+gid*4+2,ft.s2 + last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s2);
            atomic_xchg(inter+gid*4+3,ft.s3 + last_partial_sums[NB_LSIZE_1+NB_LSIZE_P].s3);
        }
    }
#endif
